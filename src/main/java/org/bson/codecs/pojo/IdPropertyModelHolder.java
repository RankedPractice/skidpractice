...
final class IdPropertyModelHolder<I> {
    ...
    static <T, I, V> IdPropertyModelHolder<I> create(Class<T> type2, PropertyModel<I> idProperty, IdGenerator<V> idGenerator) {
        if (idProperty == null && idGenerator != null) {
            throw new CodecConfigurationException(String.format("Invalid IdGenerator. There is no IdProperty set for: %s", type2));
        }
    ...
    public int hashCode() {
        int result2 = this.propertyModel != null ? this.propertyModel.hashCode() : 0;
        result2 = 31 * result2 + (this.idGenerator != null ? this.idGenerator.hashCode() : 0);
        return result2;
    }
}
