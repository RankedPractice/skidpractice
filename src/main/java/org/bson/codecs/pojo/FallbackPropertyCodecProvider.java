...
implements PropertyCodecProvider {
    ...
    public <S> Codec<S> get(TypeWithTypeParameters<S> type2, PropertyCodecRegistry propertyCodecRegistry) {
        Class<S> clazz = type2.getType();
        if (clazz == this.pojoCodec.getEncoderClass()) {
            return this.pojoCodec;
        }
        return this.codecRegistry.get(type2.getType());
    }
}
