...
implements PropertyCodecProvider {
    ...
    public <T> Codec<T> get(TypeWithTypeParameters<T> type2, PropertyCodecRegistry registry) {
        if (Collection.class.isAssignableFrom(type2.getType()) && type2.getTypeParameters().size() == 1) {
            return new CollectionCodec(type2.getType(), registry.get(type2.getTypeParameters().get(0)));
        }
        return null;
    }
